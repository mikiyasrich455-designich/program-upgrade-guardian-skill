...
mod lazy_migration_tests {
    ...
    fn test_lazy_migration_v1_to_v2_success() {
        ...
        v1_data[0..8].copy_from_slice(&[0x8a, 0x1a, 0x7c, 0x2e, 0x3f, 0x4d, 0x5e, 0x6f]);
        ...
        svm.set_account(pda, create_account(&program_id, v1_data, 2_039_280));

        let ix_data = {
            let mut data = vec![0u8; 8 + 4 + 5];
            data[0..8].copy_from_slice(&[0x1a, 0x2b, 0x3c, 0x4d, 0x5e, 0x6f, 0x7a, 0x8b]);
        ...
        assert!(data.len() >= 67, "Account did not grow: expected >= 67, got {}", data.len());
    ...
    fn test_lazy_migration_idempotent() {
        ...
        // Setup v1 account, call update_profile twice
        // Assert version stays 2, no double-migration
    }
}
```
